...
export class SignupComponent {
  ...
  signupForm = new FormGroup({
    name: new FormControl('', [Validators.pattern(/^[А-Я][а-яё]+\s*$/), Validators.required]),
    lastName: new FormControl('', [Validators.pattern(/^[А-Я][а-яё]+\s*$/), Validators.required]),
    email: new FormControl('', [Validators.email, Validators.required]),
    password: new FormControl('', [Validators.pattern(/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/), Validators.required]),
